...
heapgetpage(HeapScanDesc scan, BlockNumber page)
{
	...
	ItemId		lpp;
	bool		all_visible;
	TupleDesc tupleDesc = RelationGetDescr(scan->rs_rd);
	...
	if (scan->rs_pax) 
	{
		...
		{
			scan->rs_vistuples[i] = i + 1;
		}
		scan->rs_ntuples = tupleDesc->natts;
...
scan_paxpage(HeapScanDesc scan, Page pax_dp)
{
	...
	bool valid;
	TupleDesc tupleDesc = RelationGetDescr(scan->rs_rd);
	int pax_ncols = tupleDesc->natts;
	int atts[scan->rs_nargs / 2];
	int vals[scan->rs_nargs / 2];
	int i, j;
	ItemId lpp;
	size_t header_size = sizeof(HeapTupleHeaderData);
	char *tup;
	
       
	for (i = 0; i < scan->rs_nargs / 2; i++) 
	{
		atts[i] = scan->rs_paxscan_arg[i * 2];
		vals[i] = scan->rs_paxscan_arg[(i * 2) + 1];
	}
	
	
	for (i = 0; i < pax_ncols; i++) 
	{
		valid = true;
		for (j = 0; j < scan->rs_nargs / 2; j++)
		{
			lpp = PageGetItemId(pax_dp, atts[j]);
			tup = PageGetItem((Page) pax_dp, lpp) + header_size;

			if (*(int *)(tup + i * sizeof(int)) != vals[j])
			{
				valid = false;
				break;
			}
		...
		if (valid) 
		{
			scan->rs_matchindex[match_count] = i;
			match_count++;
		}
	}

	scan->rs_nmatches = match_count;
}
...
construct_nsmtuple(Page pax_dp, int nsm_ncols, HeapTupleHeader t_header, OffsetNumber lineoff)
{
	...
	t_header->t_hoff = sizeof(HeapTupleHeaderData);
	t_header->t_ctid.ip_posid = lineoff;
	HeapTupleHeaderSetNatts(t_header, nsm_ncols);

	for (i = 1; i <= nsm_ncols; i++) 
	{
		pax_lpp = PageGetItemId(pax_dp, i);
		tup = PageGetItem(pax_dp, pax_lpp) + sizeof(HeapTupleHeaderData);

		memcpy((char *)(t_header) + t_header->t_hoff + (i - 1) * sizeof(int),
		       tup + (lineoff - 1) * sizeof(int),
		       sizeof(int));
	}
}

static void pax_heapgettup_pagemode(HeapScanDesc scan, 
				    ScanDirection dir,
				    int nkeys,
				    ScanKey key)
{
	...
	if (ScanDirectionIsForward(dir))
	{
		if (!scan->rs_inited)
		{
			...
			scan_paxpage(scan, dp); 
			
			lineindex = scan->rs_matchindex[0];
			scan->rs_retcount = 0;
			scan->rs_inited = true;
		}
		else
		{
			/* continue from previously returned page/tuple */
			page = scan->rs_cblock;		/* current page */
			lineindex = scan->rs_matchindex[scan->rs_retcount];
			heapgetpage(scan, page);
			dp = (Page) BufferGetPage(scan->rs_cbuf);
		}
		...
		linesleft = scan->rs_nmatches - scan->rs_retcount;
	...
	for (;;)
	{
		while (linesleft > 0)
		{
			int nsm_ncols = RelationGetDescr(scan->rs_rd)->natts;
			size_t nsm_tuple_size;

			nsm_tuple_size = sizeof(HeapTupleHeaderData) + nsm_ncols * sizeof(int);			

			lineoff = scan->rs_vistuples[lineindex];
			
			tuple->t_data = malloc(nsm_tuple_size);
			tuple->t_len = nsm_tuple_size;
			ItemPointerSet(&(tuple->t_self), page, lineoff); 

			construct_nsmtuple(dp, nsm_ncols, tuple->t_data, lineoff);
		...
		if (backward) 
			lineindex = scan->rs_matchindex[lines - 1];
		else
			lineindex = scan->rs_matchindex[0];
					...
					ScanKey key)
{
	...
	for (;;)
	{
		while (linesleft > 0)
		{
			...
			tuple->t_len = ItemIdGetLength(lpp);
			tuple->t_data = malloc(tuple->t_len);
			memcpy(tuple->t_data, PageGetItem((Page) dp, lpp), tuple->t_len);
...
heap_getnext(HeapScanDesc scan, ScanDirection direction)
{
	...
	{
		if (scan->rs_pax)
			pax_heapgettup_pagemode(scan, direction, scan->rs_nkeys, scan->rs_key);
		else
