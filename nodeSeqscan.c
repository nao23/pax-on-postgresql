...
static void create_paxscan_args(HeapScanDesc scandesc, List* qual);
...
static void
create_paxscan_args(HeapScanDesc scandesc, List* qual)
{	
	ListCell *l;
	int i;
	...
	foreach(l, qual)
	{
		FuncExprState *fcache = (FuncExprState *) lfirst(l);
		...
		foreach(arg, fcache->args)
		{
			...
			if (argstate->expr->type == T_Var) 
			{
				...
				scandesc->rs_paxscan_arg[i] = (int)variable->varattno; 
			}
			else if (argstate->expr->type == T_Const) 
			{
				// get const val
				Const *con = (Const *) argstate->expr;
				scandesc->rs_paxscan_arg[i] = (int)con->constvalue;
			}
			i++;
		}
...
SeqNext(SeqScanState *node)
{
	...
	/* for PAX */
	create_paxscan_args(scandesc, node->ps.qual);
