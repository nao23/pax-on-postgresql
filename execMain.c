...
#include <papi.h>
int PAPI_events[4] = {PAPI_REF_CYC, PAPI_L1_DCM, PAPI_L2_DCM, PAPI_L3_TCM};
long long results[4];
					 ...
					 ScanDirection direction, long count)
{
	...
	if (PAPI_start_counters(PAPI_events, 4) != PAPI_OK) {
                printf("error: PAPI_start_counter()\n");
        }
	...
	if (!ScanDirectionIsNoMovement(direction))
        ...
        if (PAPI_stop_counters(results, 4) != PAPI_OK) {
                printf("error: PAPI_stop_counter()\n");
        }

        printf("PAPI results\n");
        printf("====== PAPI_results ======\n");
	printf("Referece clock cycles: %lld\n", results[0]);
